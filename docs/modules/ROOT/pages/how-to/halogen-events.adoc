= Halogen Events from Hylograph Visualizations
...
When building a Halogen application with Hylograph visualizations, you need to bridge events from the D3 world back to Halogen's action system. This guide shows you how.

== The Challenge

Hylograph visualizations live in the DOM, managed by D3. Halogen manages your application state. When a user clicks or drags something in the visualization, you need to:
...
[source,purescript]
----
import Hylograph.Internal.Behavior.Types (onClick, onClickWithDatum)
...
[source,purescript]
----
import Hylograph.Internal.Behavior.Types (Behavior(..), DragBehavior(..))
...
See the Force Explorer showcase for a complete working example of Halogen + Hylograph integration with drag, zoom, and click events.
...
* xref:how-to/simple-webpage.adoc[Hylograph without Halogen] - Simpler alternative
