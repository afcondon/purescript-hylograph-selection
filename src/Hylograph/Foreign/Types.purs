-- | Opaque handles and erased types at the D3 FFI boundary.
-- |
...
-- |
-- | These are foreign and opaque, but they are *not* private: `Datum_`
-- | and `Index_` appear in the signature of every callback a consumer
-- | writes, so they have to be nameable. The header here used to redirect
-- | to a `Hylograph.Selection` module that was never written. Formerly
-- | `Hylograph.Internal.Types`, which remains as a deprecated re-export
-- | shim.
-- |
-- | This is the one module whose new name is not simply the old one with
-- | `Internal.` removed — `Hylograph.Types` would have been too generic
-- | a name for a handful of FFI handles.
module Hylograph.Foreign.Types
