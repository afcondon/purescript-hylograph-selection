...
In this tutorial, you'll create a simple visualization using Hylograph. By the end, you'll understand the core pattern: build a *tree AST* that describes your visualization, then interpret it to render.
...
- `Hylograph.AST` - Tree building functions (`elem`, `withChild`, etc.)
- `Hylograph.AST (ElementType(..))` - Element types (`SVG`, `Circle`, `Rect`, etc.)
- `Hylograph.Expr.Friendly` - Attribute helpers (`num`, `text`, `width`, `cx`, etc.)
- `Hylograph.Render` - Rendering functions (`runD3`, `select`, `renderTree`)
...
[source,purescript]
----
import Hylograph.AST (ElementType(..))  -- includes Group
