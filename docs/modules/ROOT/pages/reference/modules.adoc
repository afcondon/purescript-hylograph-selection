...
`Hylograph.HATS`::
Declarative tree DSL: `elem`, `named`, `children`, `withEnumeration`, etc.

`Hylograph.HATS.Friendly`::
User-friendly tree building: `node`, `group`, `svg`, `circle`, etc.

`Hylograph.Expr.Friendly`::
Attribute DSL: `num`, `text`, `width`, `cx`, `fill`, etc.

`Hylograph.Render`::
DOM rendering: `runD3`, `select`, `renderData`.

`Hylograph.HATS.InterpreterTick`::
Declarative tree rendering: `rerender`, `rerenderInto`.
...
Meta-visualization of HATS trees.
