...
module PSD3.HATS.InterpreterTick
  ( rerender
  , rerenderInto
  , clearContainer
  , RerenderResult
...
rerenderTree doc parent tree = do
  result <- goWithIndex parent 0 tree
  ...
  goWithIndex :: Element -> Int -> Tree -> Effect InterpResult
    ...
    Elem spec -> do
      ...
      el <- case Array.index existingChildren childIdx of
        ...
        Nothing -> do
          ...
          pure el
      childResults <- traverseWithIndex (\i child -> goWithIndex el i child) spec.children
      let combined = Array.foldl combineResults emptyResult childResults
      pure combined
