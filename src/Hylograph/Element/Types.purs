-- | Element types and rendering contexts — the vocabulary of what can be
-- | drawn.
-- |
-- | `ElementType` is needed by every `elem` call, so this is public API and
-- | not an implementation detail. `Hylograph.HATS` re-exports it, which
-- | means the common case needs no import from here at all.
-- |
-- | Formerly `Hylograph.Internal.Element.Types`, which remains as a
-- | deprecated re-export shim.
module Hylograph.Element.Types
  ( ElementType(..)
  , RenderContext(..)
  , elementContext
  ) where
