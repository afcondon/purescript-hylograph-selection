-- | Types for declarative behavior configuration.
-- |
...
-- |
-- | This is the public API. The header here used to defer to a
-- | `Hylograph.Behavior` module that was never written. `Hylograph.HATS`
-- | re-exports the most-used members (`DragConfig`, `ZoomConfig`,
-- | `HighlightClass`, `TooltipTrigger`). Formerly
-- | `Hylograph.Internal.Behavior.Types`, which remains as a deprecated
-- | re-export shim.
module Hylograph.Behavior.Types
  ( Behavior(..)
  , ZoomConfig(..)
  , DragConfig(..)
  , ScaleExtent(..)
